...
在Twitter看到一种在windows下反弹cmd shell的方法，其实重点不在于反弹shell，而在于这种文件写入的方法可以运用到某些场景中会很方便，比如RCE没有回显时，就可以通过写入一个反弹功能的可执行文件获取到shell，但是你是如何写入该文件的？当然你可能会通过echo一个有下载功能vbs脚本，远程下载该可执行文件，那么这里再介绍给你一种方法。
